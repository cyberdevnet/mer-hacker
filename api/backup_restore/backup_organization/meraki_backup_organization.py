...
def backup_restore(ARG_ORGID, NET_ID, ARG_APIKEY, SERIAL_NUM, ARG_ORGNAME):
    x_cisco_meraki_api_key = ARG_APIKEY
    meraki = MerakiSdkClient(x_cisco_meraki_api_key)
    # orgid = ARG_ORGID
    # ID = get_org_id(meraki, ARG_ORGNAME)
    # return('Starting backup on Organization: ' + ID)
    # return('Starting backup on Organization: ' + SERIAL_NUM)
    ...
    try:
        ID = get_org_id(meraki, ARG_ORGNAME)
        return('Starting backup on Organization: ' + ID + 'CIAO')
    except:
        return('nothing')
        # try:
        #     ID = get_org_id(meraki, ARG_ORGNAME)
...
def get_org_id(meraki, ARG_ORGNAME):
...
#     file.write("\n")
#     file.write("ARG_ORGNAME = 'Heisenberg'\n")
#     file.write("\n")
#     file.write("def get_org_id(ARG_ORGNAME):\n")
#     file.write("\ttry:\n")
#     file.write(
...
#     myRules = meraki.mx_vpn_firewall.get_organization_vpn_firewall_rules(orgid)[
#         0:-1]
...
#             switchports = meraki.switch_ports.get_device_switch_ports(
#                 row['serial'])
#     for port in switchports:
#         file.write("\tputurl = 'https://api.meraki.com/api/v0/networks/{0}/devices/"+str(
#             row['serial'])+"/switchPorts/"+str(port['number'])+"'.format(str(networkid))\n")
#         file.write("\tdashboard = session.put(puturl, json=" +
#                    str(port)+", headers=headers)\n")
...
# # parser.add_argument('ARG_ORGNAME', help='The name of a Meraki organisation')
# # args = parser.parse_args()
# ARG_ORGNAME = "Heisenberg"
...
#     myNetworks = meraki.networks.get_organization_networks(
...
#         # jsonModel.update({"type": row['productTypes'],'name': row['name'],"disableMyMerakiCom": row['disableMyMerakiCom'],"disableRemoteStatusPage": row['disableMyMerakiCom']})
...
#                    row['name']+" - it probably already exists')\n")
#         file.write("\n")
#         file.flush()
