...
def getShardHost(p_org):
    ...
    merakirequestthrottler()
    dashboard = meraki.DashboardAPI(ARG_APIKEY, output_log=False)
    organizationsSnmp = dashboard.snmp_settings.getOrganizationSnmp(p_org.id)
    ...
    for org in ORG_LIST:
        for net in org.nets:
            form.netname.choices.append(('%s|%s|%s|%s|%s' % (
                org.id, org.shard, net.id, net.mxsn1, net.mxsn2), '%s [%s]' % (net.name, org.name)))
...
def main(argv):
    ...
    if arg_mode.lower() == 'http':
        ...
        if os.path.exists('cert.pem') and os.path.exists('key.pem'):
            app.run(host='0.0.0.0', port=SERVER_HTTPS_PORT,
