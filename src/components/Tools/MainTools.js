...
export default function MainTools(ac) {
  return (
    <div>
      ...
      {ac.dc.switchAllTools[6] ? <NetworkTopUsers dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[7] ? <FindPorts dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[8] ? <TrafficAnalysis dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[9] ? <BackupRestore dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[10] ? <MigrateTool dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[11] ? <SwitchPortTemplate dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[12] ? <ChangeLog dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[13] ? <GetInventory dc={ac.dc} /> : <div></div>}
      {ac.dc.switchAllTools[14] ? <DeployDevices dc={ac.dc} /> : <div></div>}
    </div>
  );
}
