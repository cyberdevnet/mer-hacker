...
# This will create a file called meraki-restore-network.py.  To do a restore you go:
# meraki-restore-network.py "org name"
# Note that the store will not overwrite existing networks.  You can either rename an existing network you want to restore
# over or edit meraki-restore-network.py to restore into a different [new] network.  Also you can edit meraki-restore-network.py to only
...
def write_restore_header(file):
	...
	file.write("\n");
	file.write("headers = {\n");
	file.write("\t'x-cisco-meraki-api-key': 'd5c1be108a9dc2c89ea4ba757abc2355b704e774',\n");
	file.write("\t'Content-Type': 'application/json'\n");
	file.write("\t}\n");
	file.write("\n");
	file.write("session = requests.Session()\n")
	file.write("\n");
	file.write("orgName = 'Heisenberg'\n")
	file.write("\n");
...
x_cisco_meraki_api_key = 'd5c1be108a9dc2c89ea4ba757abc2355b704e774'
meraki = MerakiSdkClient(x_cisco_meraki_api_key)
orgid = 764320

with open('meraki-restore-network.py', 'w') as file:
	...
	network_id = 'L_681169443639788368'
	myNetwork = meraki.networks.get_network(network_id)
	row = (myNetwork)
